  ...
  methods: {
    getTableData() {
      getHome().then((res) => {
        ...
        //用户图
        this.ehcartData.user.xAxis = res.data.userData.map((item) => item.date);

        this.ehcartData.user.series.push({
          name: "新增用户",
          data: res.date.userData.map((item) => item.new),
          type: "bar",
        });
        this.ehcartData.user.series.push({
          name: "活跃用户",
          data: res.date.userData.map((item) => item.active),
          type: "bar",
        });
        ehcartData.setOption(this.ehcartData.user);
